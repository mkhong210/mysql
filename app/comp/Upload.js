"use client"

import axios from 'axios';
import React, { useEffect, useState } from 'react'

function Upload() {
	const [imageView, setImageView] = useState();
	const [data, setData] = useState([]);
	const [testBlob, setTestBlob] = useState();

	const uploadFile = (e) => {
		e.preventDefault();
		// 두개 세트로 써야만 데이터가 나옴 
		const formdata = new FormData(e.target);
		const obj = Object.fromEntries(formdata);
		// console.log(obj.upload)

		const fr = new FileReader();
		fr.readAsDataURL(obj.upload);
		fr.addEventListener('load', () => {
			// 이미지 명 => obj.title
			// 이미지 => obj.result
			// console.log(fr.result)
			axios.post('/api/upload/files', {
				title: obj.title,
				imgUrl: fr.result
			})
		})
	}

	const getFile = async () => {
		const d = await axios.get('/api/upload/files')
		// console.log(d.data);
		
		const setD = d.data.map (obj => {
			obj.imgUrl = base64toBlob(obj.imgUrl);
			return obj;
		})
		setData(setD);
	}

	// bas64를 blob으로 변환해주는 함수 - 웹 최적화를 위한 코드변환
	function base64toBlob(b64Data, contentType='') {
		const image_data = atob(b64Data.split(',')[1]); // data:image/gif;base64 필요없으니 떼주고, base64 인코딩을 풀어준다

		const arraybuffer = new ArrayBuffer(image_data.length);
		const view = new Uint8Array(arraybuffer);

		for (let i = 0; i < image_data.length; i++) {
			view[i] = image_data.charCodeAt(i) & 0xff;
			// charCodeAt() 메서드는 주어진 인덱스에 대한 UTF-16 코드를 나타내는 0부터 65535 사이의 정수를 반환
			// 비트연산자 & 와 0xff(255) 값은 숫자를 양수로 표현하기 위한 설정
		}

		const blob = new Blob([arraybuffer], { type: contentType });
		return URL.createObjectURL(blob); // object url 생성
		// const blobUrl = URL.createObjectURL(blob); // object url 생성
		
		// console.log(blobUrl);
		// return new Blob([arraybuffer], { type: contentType });
	}

	useEffect(() => {
		getFile();
	}, [])

	return (
		<div>
			<h2>파일 업로드</h2>
			<form onSubmit={uploadFile} method='post' encType='multipart/form-data'>
				<p><input type="text" name="title" /></p>
				<p>
					<input type="file" name="upload" onChange={(e) => {
						// e.target - input태그 
						// e.target.files - 배열 형태로 이미지에 대한 정보가 들어옴
						// e.target.files[0] - 파일 정보만 들어옴 
						// console.log(e.target.files[0]);
						const file = e.target.files[0]
						// URL.createObjectURL(file) - file을 볼수있는 url 생성
						// file - 파일올리다 취소 / null 등 일떄 오류 안나게
						file && setImageView(URL.createObjectURL(file))
					}} />
					{/* <img width="200" src="blob:http://localhost:3000/12522cd8-7cdf-4eee-a283-f3242607872c"></img> */}
					<img src={imageView} width='200' />
				</p>
				<p><input type="submit" value="저장" /></p>
			</form>
			<div>
				{
					data.map(obj => (
						<figure key={obj.num}>
							<img src={obj.imgUrl} alt={obj.title} width="200" />
							<figcaption>{obj.title}</figcaption>
						</figure>
					))
				}
			</div>
		</div>
	)
}

export default Upload